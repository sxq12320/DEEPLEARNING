"""
FCN (Fully Convolutional Network) 实现
包含 FCN-32s, FCN-16s, FCN-8s 三个版本
骨干网络：手写 VGG16，从零训练，不依赖任何预训练权重
"""

import torch
import torch.nn as nn


# ──────────────────────────────────────────────────────────────
# VGG16 骨干网络（手写，从零训练）
# ──────────────────────────────────────────────────────────────

def _conv_block(in_ch, out_ch, num_convs):
    """
    VGG 基本单元：num_convs 个 Conv-BN-ReLU + MaxPool
    原论文没有 BN，但从零训练加 BN 收敛更稳定
    """
    layers = []
    for _ in range(num_convs):
        layers += [
            nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
        ]
        in_ch = out_ch
    layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
    return nn.Sequential(*layers)


class VGG16Backbone(nn.Module):
    """
    手写 VGG16 特征提取，分5段输出：
        pool1: H/2  × W/2   通道 64
        pool2: H/4  × W/4   通道 128
        pool3: H/8  × W/8   通道 256   ← FCN-8s 跳跃连接
        pool4: H/16 × W/16  通道 512   ← FCN-16s 跳跃连接
        pool5: H/32 × W/32  通道 512   ← 主干输出
    """
    def __init__(self):
        super().__init__()
        self.pool1 = _conv_block(3,   64,  2)
        self.pool2 = _conv_block(64,  128, 2)
        self.pool3 = _conv_block(128, 256, 3)
        self.pool4 = _conv_block(256, 512, 3)
        self.pool5 = _conv_block(512, 512, 3)
        self._init_weights()

    def forward(self, x):
        x  = self.pool1(x)
        x  = self.pool2(x)
        p3 = self.pool3(x)
        p4 = self.pool4(p3)
        p5 = self.pool5(p4)
        return p3, p4, p5

    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out',
                                        nonlinearity='relu')
            elif isinstance(m, nn.BatchNorm2d):
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias,   0)


# ──────────────────────────────────────────────────────────────
# FC6 / FC7：全连接层卷积化
# ──────────────────────────────────────────────────────────────

def _make_fc_conv():
    """VGG FC6+FC7 卷积化，kernel=1 适配任意输入尺寸"""
    return nn.Sequential(
        nn.Conv2d(512, 4096, kernel_size=1, bias=True),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=0.5),
        nn.Conv2d(4096, 4096, kernel_size=1, bias=True),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=0.5),
    )

def _init_fc_conv(module):
    for m in module.modules():
        if isinstance(m, nn.Conv2d):
            nn.init.kaiming_normal_(m.weight, mode='fan_out',
                                    nonlinearity='relu')
            nn.init.constant_(m.bias, 0)


# ──────────────────────────────────────────────────────────────
# 工具函数
# ──────────────────────────────────────────────────────────────

def _bilinear_init(layer: nn.ConvTranspose2d):
    """双线性插值初始化转置卷积（论文 3.3 节）"""
    w = layer.weight.data
    f = w.shape[2]
    c = (2 * f - 1 - f % 2) / (2.0 * f)
    for i in range(f):
        for j in range(f):
            val = (1 - abs(i / f - c)) * (1 - abs(j / f - c))
            w[:, :, i, j] = val

def _crop(x: torch.Tensor, target_size):
    """中心裁剪，消除转置卷积输出与目标尺寸的偏差"""
    h, w   = target_size
    oh, ow = x.shape[2], x.shape[3]
    dh = (oh - h) // 2
    dw = (ow - w) // 2
    return x[:, :, dh:dh + h, dw:dw + w]


# ──────────────────────────────────────────────────────────────
# FCN-32s
# ──────────────────────────────────────────────────────────────

class FCN32s(nn.Module):
    """pool5 → FC → score → 32× 上采样 → 输出"""
    def __init__(self, num_classes: int):
        super().__init__()
        self.backbone  = VGG16Backbone()
        self.fc_conv   = _make_fc_conv()
        self.score_fr  = nn.Conv2d(4096, num_classes, kernel_size=1)
        self.upscore32 = nn.ConvTranspose2d(
            num_classes, num_classes,
            kernel_size=64, stride=32, padding=16, bias=False
        )
        self._init_weights()

    def forward(self, x):
        input_size    = x.shape[2:]
        _, _, p5      = self.backbone(x)
        x = self.fc_conv(p5)
        x = self.score_fr(x)
        x = self.upscore32(x)
        return _crop(x, input_size)

    def _init_weights(self):
        _init_fc_conv(self.fc_conv)
        nn.init.kaiming_normal_(self.score_fr.weight)
        nn.init.constant_(self.score_fr.bias, 0)
        _bilinear_init(self.upscore32)


class FCN16s(nn.Module):
    """
    pool5 → 2× 上采样  ┐
                        ├─ 相加 → 16× 上采样 → 输出
    pool4 → 1×1 conv   ┘
    """
    def __init__(self, num_classes: int):
        super().__init__()
        self.backbone    = VGG16Backbone()
        self.fc_conv     = _make_fc_conv()
        self.score_fr    = nn.Conv2d(4096, num_classes, kernel_size=1)
        self.score_pool4 = nn.Conv2d(512,  num_classes, kernel_size=1)
        self.upscore2    = nn.ConvTranspose2d(
            num_classes, num_classes,
            kernel_size=4, stride=2, padding=1, bias=False
        )
        self.upscore16   = nn.ConvTranspose2d(
            num_classes, num_classes,
            kernel_size=32, stride=16, padding=8, bias=False
        )
        self._init_weights()

    def forward(self, x):
        input_size = x.shape[2:]
        _, p4, p5  = self.backbone(x)

        x  = self.fc_conv(p5)
        x  = self.score_fr(x)           # stride=32
        x  = self.upscore2(x)           # stride=32 → stride=16

        s4 = self.score_pool4(p4)
        s4 = _crop(s4, x.shape[2:])
        x  = x + s4                     # 融合 pool4

        x  = self.upscore16(x)
        return _crop(x, input_size)

    def _init_weights(self):
        _init_fc_conv(self.fc_conv)
        nn.init.kaiming_normal_(self.score_fr.weight)
        nn.init.constant_(self.score_fr.bias, 0)
        nn.init.constant_(self.score_pool4.weight, 0)   # 初始为0（论文要求）
        nn.init.constant_(self.score_pool4.bias,   0)
        _bilinear_init(self.upscore2)
        _bilinear_init(self.upscore16)


class FCN8s(nn.Module):
    """
    pool5 → 2× 上采样  ┐
                        ├─ 相加 → 2× 上采样  ┐
    pool4 → 1×1 conv   ┘                     ├─ 相加 → 8× 上采样 → 输出
                          pool3 → 1×1 conv   ┘
    """
    def __init__(self, num_classes: int):
        super().__init__()
        self.backbone     = VGG16Backbone()
        self.fc_conv      = _make_fc_conv()
        self.score_fr     = nn.Conv2d(4096, num_classes, kernel_size=1)
        self.score_pool4  = nn.Conv2d(512,  num_classes, kernel_size=1)
        self.score_pool3  = nn.Conv2d(256,  num_classes, kernel_size=1)
        self.upscore2      = nn.ConvTranspose2d(
            num_classes, num_classes,
            kernel_size=4, stride=2, padding=1, bias=False
        )
        self.upscore_pool4 = nn.ConvTranspose2d(
            num_classes, num_classes,
            kernel_size=4, stride=2, padding=1, bias=False
        )
        self.upscore8      = nn.ConvTranspose2d(
            num_classes, num_classes,
            kernel_size=16, stride=8, padding=4, bias=False
        )
        self._init_weights()

    def forward(self, x):
        input_size = x.shape[2:]
        p3, p4, p5 = self.backbone(x)

        x  = self.fc_conv(p5)
        x  = self.score_fr(x)             # stride=32

        # ── 第一次融合：pool5 + pool4 ──
        x  = self.upscore2(x)             # stride=32 → stride=16
        s4 = self.score_pool4(p4)
        s4 = _crop(s4, x.shape[2:])
        x  = x + s4

        # ── 第二次融合：(pool5+pool4) + pool3 ──
        x  = self.upscore_pool4(x)        # stride=16 → stride=8
        s3 = self.score_pool3(p3)
        s3 = _crop(s3, x.shape[2:])
        x  = x + s3

        # ── 最终 8× 上采样 ──
        x  = self.upscore8(x)
        return _crop(x, input_size)

    def _init_weights(self):
        _init_fc_conv(self.fc_conv)
        nn.init.kaiming_normal_(self.score_fr.weight)
        nn.init.constant_(self.score_fr.bias, 0)
        nn.init.constant_(self.score_pool4.weight, 0)
        nn.init.constant_(self.score_pool4.bias,   0)
        nn.init.constant_(self.score_pool3.weight, 0)
        nn.init.constant_(self.score_pool3.bias,   0)
        _bilinear_init(self.upscore2)
        _bilinear_init(self.upscore_pool4)
        _bilinear_init(self.upscore8)


# ──────────────────────────────────────────────────────────────
# 工厂函数
# ──────────────────────────────────────────────────────────────

def build_model(model_type: str, num_classes: int) -> nn.Module:
    """model_type: 'fcn32s' | 'fcn16s' | 'fcn8s'"""
    model_map = {
        'fcn32s': FCN32s,
        'fcn16s': FCN16s,
        'fcn8s':  FCN8s,
    }
    assert model_type in model_map, f"未知模型类型: {model_type}"
    return model_map[model_type](num_classes)


if __name__ == '__main__':
    x = torch.randn(2, 3, 512, 512)
    for name in ['fcn32s', 'fcn16s', 'fcn8s']:
        model = build_model(name, num_classes=21)
        out   = model(x)
        params = sum(p.numel() for p in model.parameters()) / 1e6
        print(f"{name}: input {list(x.shape)} → output {list(out.shape)}  参数量: {params:.1f}M")
