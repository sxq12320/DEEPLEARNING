...
class Conv2d_BN(torch.nn.Sequential):
    ...
    Args:
        ...
        dilation : 扩张率
        groups : 分组数
        bn_weight_init : 批量归一化权重初始化值
    Returns:
        torch.nn.Sequential 一组torch的nn类别运算
    '''
    ...
    @torch.no_grad() # 关闭自动梯度计算器
    def fuse(self):
        '''
        using:
            融合函数,返回一个全新的卷积层torch.nn.Conv2d
        Args:
            None
        Returns:
            torch.nn.Conv2d : 融合后的卷积层
        '''
        c, bn = self._modules.values() # 返回当前容器中的全部子模块的参数
