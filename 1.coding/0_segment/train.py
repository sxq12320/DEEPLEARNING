...
def parse_args(argv=None):
    ...
    parser.add_argument("--save-cfg", type=str, default="",
    ...
    parser.add_argument("--label-type", type=str, default=None,
                        choices=["mask", "txt", "json", "npy"])
...
def train(cfg):
    ...
    for epoch in range(cfg["epochs"]):
        epoch_loss = 0.0
        pbar = tqdm(loader, desc=f"Epoch {epoch + 1}/{cfg['epochs']}", unit="batch", leave=False)
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(f"{' ' * 4}{epoch + 1:>5}/{cfg['epochs']:<5}  loss={avg_loss:.6f}\n")
