...
import numpy as np
import cv2
from data.transforms.image_transform import image_transform
from data.preprocessing.enhance_image import enhance_image
from torchvision import transforms
from data.transforms.label_transform import TXT2MASK, JSON2MASK, NPY2MASK
...
class get_dataset_rgb(Dataset):
    ...
    def __init__(
        ...
        Args:
            image_dir (str): 图像目录。
            label_dir (str): 标签目录。
            label_type (str): 标签类型, 支持 mask/txt/json/npy。
            target_size (tuple): 目标尺寸, 格式为 (W, H)。
        '''
        super().__init__()
    ...
    def __getitem__(
        ...
        Returns:
            Tuple[torch.Tensor, torch.Tensor]:
                - image: 形状通常为 (3, H, W) 的图像张量。
                - label: 形状通常为 (H, W, 1) 的标签张量。
        '''
