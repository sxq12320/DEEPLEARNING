...
深度学习计算机视觉研究与实践仓库，涵盖**图像分割**、**目标检测（RGB-D 苹果遮挡检测）**与**经典网络架构学习**三大方向。
...
基于 PyTorch 的模块化工程，支持**图像分割**与**目标检测**两大任务。采用**注册机制 + 配置驱动组网**，可灵活更换 backbone / neck / head。
...
详见 [1.coding/0_segment/README.md](1.coding/0_segment/README.md)。
...
- [2_Unet](1.coding/2_Unet/) — U-Net 医学图像分割实现（含训练图像样本）
- [3_phics_x](1.coding/3_phics_x/) — PHICS-X 模型，基于 ResNet18 基座

---

## 2. 2_catoon — Manim 数学动画

使用 [Manim](https://www.manim.community/) 制作神经网络原理动画，用于可视化教学（如 LeNet 结构演示）。

---

## 3. ultralytics-main-new — RGB-D 苹果遮挡检测

基于 Ultralytics YOLO11 的定制版本，采用**纯 CNN + 频域 + 动态门控**机制（不依赖 Transformer / Mamba），面向果园 RGB-D 苹果遮挡检测。
...
- [mine_yaml/](ultralytics-main-new/mine_yaml/) — 11 组消融与对比实验（baseline → 完整模型）
- [mine_yaml_v4/](ultralytics-main-new/mine_yaml_v4/) — V4 版本 10 组实验
- [results/](ultralytics-main-new/results/) — 训练结果（曲线、混淆矩阵、预测样例）

详见 [ultralytics-main-new/README.md](ultralytics-main-new/README.md)。
...
仓库配置了 GitHub Actions（[.github/workflows/main.yml](.github/workflows/main.yml)）：当带有 `blog` 标签的 Issue 被创建/编辑时，自动将标题与链接追加到下方列表。
