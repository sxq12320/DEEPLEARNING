...
当前服务器数据位于 `E:\mastercode\data\orange_yolo`，实际文件计数如下。
...
python scripts\prepare_dataset.py `
  --source E:\mastercode\data\orange_yolo `
...
| `citrus-maskrcnn` | 3.10 | Torchvision Mask R-CNN |
...
正式训练前，每个模型先运行 `2` epoch，并确认 GPU 被使用、训练损失下降、验证 AP 不是零、预测掩膜位置正确。Mask R-CNN 使用纯 PyTorch/Torchvision，可在 Windows 或 Linux 运行；正式实验仍建议放到 Linux GPU 服务器。
...
本项目采用 PyTorch 官方 Torchvision 实现，不再为该基线安装 MMDetection、MMCV 或 Detectron2。先安装与服务器 CUDA 驱动匹配的 `torch + torchvision`，再安装其余依赖：

```bash
conda create -n citrus-maskrcnn python=3.10 -y
conda activate citrus-maskrcnn
# 以下仅以 CUDA 12.8 为例，实际命令按 pytorch.org 安装选择器生成
pip install torch torchvision --index-url https://download.pytorch.org/whl/cu128
pip install -r requirements-torchvision-maskrcnn.txt
```

Torchvision 内部的 `0` 是背景，转换后的幼果 `category_id` 必须为 `1`。训练脚本加载官方 COCO 权重后自动替换分类头和 Mask 头，不需要手工修改模型源码。

```bash
python scripts/train_torchvision_maskrcnn.py \
  --dataset datasets/citrus_prepared \
  --name E_maskrcnn_r50_fpn_seed42 \
  --epochs 300 --batch 2 --lr 0.005 --imgsz 640 --seed 42
...
详细的数据转换、环境检查、冒烟训练、续训、最终测试和故障排查见
`Mask_R-CNN_Torchvision实例分割部署与数据转换指南.md`。
