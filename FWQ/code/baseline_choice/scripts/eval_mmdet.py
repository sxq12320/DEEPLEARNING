...
def main() -> None:
    ...
    image_count = 0
    ordered_images = sorted(image_index.items(), key=lambda item: int(item[1]["id"]))
    for file_name, record in tqdm(ordered_images, desc=f"MMDetection {args.split}", unit="image", dynamic_ncols=True):
