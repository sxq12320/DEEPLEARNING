...
def parse_label_file(path: Path, num_classes: int = 1) -> List[Polygon]:
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if len(fields) < 7 or (len(fields) - 1) % 2:
            raise ValueError(
                f"{path}:{line_number}: expected class plus at least three x/y points"
            )
        class_id = int(fields[0])
        if not 0 <= class_id < num_classes:
            raise ValueError(
                f"{path}:{line_number}: class id {class_id} is outside [0, {num_classes})"
            )
...
def source_split_directories(source_root: Path, split: str) -> Tuple[Path, Path]:
    """Resolve either images/<split> or <split>/images YOLO layouts."""
    candidates = (
        (source_root / "images" / split, source_root / "labels" / split),
        (source_root / split / "images", source_root / split / "labels"),
    )
    matches = [
        (images, labels)
    ...
    if not matches:
        expected = " or ".join(f"({images}, {labels})" for images, labels in candidates)
        raise FileNotFoundError(
            f"Missing images/labels directories for split '{split}'. Expected {expected}"
        )
    if len(matches) > 1:
        raise ValueError(
            f"Both supported YOLO layouts exist for split '{split}' under {source_root}; keep only one layout"
        )
    return matches[0]
...
def convert_split(
    ...
    source_images, source_labels = source_split_directories(source_root, split)

    images = image_files(source_images)
    stems = [path.stem for path in images]
    if len(stems) != len(set(stems)):
        raise ValueError(
            f"Duplicate image stems in {source_images}; labels are stem-based"
        )
    ...
    for image_path in tqdm(
        ...
        if not label_path.exists():
            raise FileNotFoundError(
                f"Missing label file for {image_path.name}: {label_path}"
            )
        ...
        mask_path = (
            output_root / "semantic" / "masks" / split / f"{image_path.stem}.png"
        )
    ...
    categories = [
        {"id": index + 1, "name": name, "supercategory": "citrus"}
        for index, name in enumerate(class_names)
    ]
    coco = {
        "info": {
            "description": "Immature citrus instance segmentation baseline dataset"
        },
        "licenses": [],
        "images": coco_images,
...
def write_format_metadata(
    ...
    for split, coco in split_coco.items():
        ...
        save_json(
            output_root / "rfdetr" / rfdetr_split / "_annotations.coco.json", coco
        )
...
def prepare_dataset(
    ...
    summary = {
        ...
        "totals": {
            ...
            "negative_images": sum(
                int(report["negative_images"]) for report in split_reports
            ),
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Source root using images/<split> or <split>/images YOLO layout.",
    )
    parser.add_argument(
        "--output", type=Path, required=True, help="Output dataset root."
    )
    parser.add_argument(
        "--class-name",
        action="append",
        default=None,
        help="Class name; repeat for multiple classes.",
    )
    parser.add_argument(
        "--splits", nargs="+", default=list(DEFAULT_SPLITS), help="Splits to convert."
    )
    parser.add_argument(
        "--mode",
...
def main() -> None:
    ...
    summary = prepare_dataset(
        args.source, args.output, class_names, args.splits, args.mode
    )
