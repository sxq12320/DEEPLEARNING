...
#         总体由两部分组成：
#         1) 参数一致性损失：预测掩码拟合椭圆参数 与 GT 椭圆参数保持一致。
#         2) 软椭圆 Dice 损失：预测掩码与 GT 椭圆渲染图保持形状一致。
#         """
#         if self.ellipse_param_weight <= 0 and self.ellipse_dice_weight <= 0:
#             # 权重为 0 时直接跳过，不影响原有训练行为
#             return gt_mask.new_zeros(())

#         # 由 mask 系数和 proto 重建每个实例掩码
#         pred_logits = torch.einsum("in,nhw->ihw", pred, proto)
#         pred_prob = pred_logits.sigmoid().clamp(self.ellipse_eps, 1.0 - self.ellipse_eps)
#         gt_prob = gt_mask.float().clamp(self.ellipse_eps, 1.0)
...
#         # 参数一致性：中心 + 轴长(log 域更稳定) + 角度(sin/cos 避免角度跳变)
#         center_loss = (cx_p - cx_g).abs() + (cy_p - cy_g).abs()
#         axis_loss = (torch.log(a_p + self.ellipse_eps) - torch.log(a_g + self.ellipse_eps)).abs() + (
#             torch.log(b_p + self.ellipse_eps) - torch.log(b_g + self.ellipse_eps)
#         ).abs()
...
#         # 软椭圆先验：预测掩码应接近 GT 拟合出的椭圆形状
#         ellipse_gt = self._soft_ellipse_mask(
#             cx_g, cy_g, a_g, b_g, t_g, gt_crop.shape[-2], gt_crop.shape[-1], self.ellipse_softness, self.ellipse_eps
#         )
#         inter = (pred_crop * ellipse_gt).sum(dim=(1, 2))
#         union = pred_crop.sum(dim=(1, 2)) + ellipse_gt.sum(dim=(1, 2))
#         dice_loss = 1.0 - (2.0 * inter + self.ellipse_eps) / (union + self.ellipse_eps)
