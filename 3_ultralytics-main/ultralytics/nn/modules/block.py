...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        x_cat = torch.cat([avg_out, max_out], dim=1)
        out = self.sigmoid(self.conv1(x_cat))
        return out


class MyCBAM(nn.Module):
    """
    完整的 CBAM 模块，适配 YOLO11 解析器格式。
    """
    ...
    def __init__(self, c1, c2, ratio=16, kernel_size=7):
        ...
        # 如果 YOLO YAML 配置文件里要求输入输出通道改变，强制对齐
        if c1 != c2:
            self.align_conv = nn.Conv2d(c1, c2, 1, 1, bias=False)
            self.bn = nn.BatchNorm2d(c2)
            self.act = nn.SiLU()
    ...
    def forward(self, x):
        if self.c1 != self.c2:
            x = self.act(self.bn(self.align_conv(x)))
        x = x * self.ca(x)
        x = x * self.sa(x)
        return x
...
class DWSConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        if k == 1:
            # 1×1 直接走标准 Conv，没有意义再做 DW
            self.dw = None
            self.pw = Conv(c1, c2, 1, s, p, g, d, act)
        else:
            # DW：groups=c1，每通道独立空间卷积
            self.dw = nn.Sequential(
                nn.Conv2d(c1, c1, k, s, autopad(k, p, d),
