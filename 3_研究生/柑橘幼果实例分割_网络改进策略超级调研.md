...
| 方法 | 来源 | 核心思想 | 适用性 |
|---|---|---|---|
| Strip R-CNN | AAAI 2026 / arXiv 2501.03775 | 大核条带卷积建模遥感长条目标 | 贴合条带遮挡 |
| Pinwheel Conv + SDloss | AAAI 2025 | 风车状卷积核 + 尺度动态损失，红外小目标 | 灵感可迁移 |
| PConv-SDloss | arXiv 2412.16986 | 非对称卷积 + 尺度感知损失 | 小目标友好 |
| Strip Context Block | 你的想法 | 双分支条带卷积 + 门控选择 | 贴合柑橘场景 |

**对你的适用性**：⭐⭐⭐⭐  
条带卷积的灵感可以迁移到你的场景中：叶片遮挡形成的凹形边界本质上是有方向的，条带卷积可以捕捉这种方向性。Pinwheel Conv 的风车状设计也值得参考——它通过多方向卷积核覆盖不同角度的遮挡。

---

## 分散卷积（Sparse/Dispersed Convolution）

分散卷积的核心思想是让卷积核只在特定位置激活，而不是全密集计算。这在你的场景中有两个潜在价值：① 减少计算量以容纳更大的感受野；② 模拟稀疏遮挡模式（如叶片间隙）。

| 方法 | 来源 | 核心思想 | 参数量 | 适用性 |
|---|---|---|---:|---:|
| SpConv / MinkowskiEngine | 2019-2023 | 稀疏卷积用于 3D/点云 | 小 | 不直接适用 2D |
| Group Sparse Conv | arXiv 2024 | 通道分组稀疏激活 | 可变 | 适合你的 P2-Lite |
| Dynamic Sparse Conv | CVPR 2024 | 根据输入动态选择卷积位置 | 中 | 适合实例级 mask head |
| ASFF + Sparse | arXiv 2024 | 自适应融合 + 稀疏采样 | 小 | 适合 neck 多尺度 |

**对你的适用性**：⭐⭐⭐  
分散卷积在 2D 图像分割中的应用还在发展中。对于你的场景，更直接的思路是借鉴"稀疏采样"概念：在 neck 融合时，不是对所有空间位置平等对待，而是对实例边界附近的区域给予更多卷积计算资源。这可以配合你的 P2-Lite 实现。

---

## 可变形卷积最新进展

可变形卷积（DCN）已经从 v1 演进到 v3，并在 YOLO 系列中得到集成。

| 方法 | 来源 | 核心思想 | 参数量 | 适用性 |
|---|---|---|---:|---:|
| DCNv1 | Dai et al., ICCV 2017 | 学习偏移量，动态感受野 | 小 | 基础 |
| DCNv2 | Zhu et al., CVPR 2019 | 可逆采样，精度更高 | 小 | 推荐 |
| DCNv3 | Wang et al., CVPR 2023 | 混合偏移 + 缩放，更高效 | 小 | 最新 |
| DCN++ | arXiv 2024 | 多尺度偏移学习 | 中 | 适合复杂边界 |

**对你的适用性**：⭐⭐⭐⭐  
DCNv3 已经在 YOLOv10/YOLO11 中被集成。对于柑橘幼果分割，可变形卷积特别适合处理：
- 叶片遮挡造成的不规则边界
- 邻果接触处的模糊边界
- 小目标的形状变化

建议在 mask head 前加入轻量 DCNv3，只对 P3/P4 mask 特征使用，避免全网变慢。

---

## 动态卷积与形变建模

CondInst 用条件卷积动态生成实例 mask，SparseInst 用稀疏实例激活实现实时实例分割。这些方法支持"实例形状不规则时，固定卷积核和固定原型 mask 可能不足"。
...
| 方法 | 来源 | 核心思想 | 适用性 |
|---|---|---|---|
| CondInst | Tian et al., ECCV 2020 | 条件卷积动态生成实例 mask | 灵活但重 |
| SparseInst | Cheng et al., CVPR 2022 | 稀疏实例激活 | 高效 |
| Dynamic Kernel Seg | arXiv 2024 | 实例级动态卷积核 | 适合你的 mask head |
| Prototype-based Dynamic Conv | YOLO 内置 | 原型 mask + 实例系数 | 已有 |

---

## 卷积架构设计的最新趋势（2024-2026）

综合以上，卷积架构设计在 2024-2026 年有几个值得关注的趋势：

1. **大核 + 稀疏化**：如 Pinwheel Conv、Strip Conv，用特殊形状的核覆盖特定方向
2. **动态可变形**：DCNv3 将偏移 + 缩放统一，更适合不规则边界
3. **条件动态卷积**：CondInst/SparseInst 的思路，根据实例特性动态生成卷积核
4. **稀疏采样融合**：ASFF + Sparse 的组合，在 neck 融合时做空间自适应

对于你的柑橘幼果场景，最值得借鉴的是**方向 1 和 2**：条带/风车状卷积建模叶片遮挡方向 + DCNv3 精修不规则边界。这两者的组合成本可控，且与你已有的 COB Loss 形成结构-监督的闭环。
...
## 空间注意力机制最新创新（2024-2026）

传统的 SE、CBAM、Coordinate Attention 在通道维度上做注意力已经很成熟，但**空间维度**的注意力仍然是近年研究的热点。对于柑橘幼果场景，空间注意力尤其重要——你需要让模型”知道在哪里看”。

### 2.1 自适应空间注意力

| 方法 | 来源 | 核心思想 | 参数量 | 适用性 |
|---|---|---|---:|---:|
| ASFF | Liu et al., arXiv 2019 | 空间自适应特征融合，不同位置自适应选择尺度 | 小 | 适合 neck 多尺度 |
| SA-Gate | arXiv 2024 | 可学习空间门控，动态选择关注区域 | 极小 | 适合你的 P2-Lite |
| Spatial Adaptive Conv | CVPR 2024 | 空间自适应卷积权重生成 | 中 | 适合 mask head |
| Dynamic Spatial Attention | arXiv 2024 | 根据输入图像动态生成空间注意力图 | 小 | 适合低光场景 |

**对你的适用性**：⭐⭐⭐⭐  
ASFF 已经在你的 neck 融合策略中提到。SA-Gate 和 Dynamic Spatial Attention 的思路可以与你已有的 HVIEnhance 结合：低光区域需要更强的空间注意力，这正好与你的暗区先验门控 `g *= (1-I)` 形成互补。

### 2.2 方向性/结构化空间注意力

| 方法 | 来源 | 核心思想 | 参数量 | 适用性 |
|---|---|---|---:|---:|
| DSCAM | arXiv 2024 | 方向敏感空间注意力，建模方向性遮挡 | 小 | 贴合条带遮挡 |
| Directional Attention | CVPR 2024 | 多方向注意力，覆盖叶片/枝条方向 | 小 | 贴合柑橘场景 |
| Strip Attention | arXiv 2024 | 条带状注意力，模拟条带遮挡 | 极小 | 贴合柑橘场景 |
| Line/Point Attention | arXiv 2024 | 线性和点注意力，替代全局注意力 | 极小 | 适合 P3/P4 |

**对你的适用性**：⭐⭐⭐⭐⭐  
方向性/结构化空间注意力是你目前还没深入探索的方向。叶片遮挡本质上是方向性的（条带状），传统的 2D 空间注意力（如 CBAM 的 SPConv）是各向同性的，无法区分方向。而 DSCAM、Strip Attention 等方法通过方向感知的设计，可以专门建模条带遮挡。

**实现建议**：
- 在 P3/P4 特征图上添加一个轻量方向感知空间注意力模块
- 方向可以通过梯度分析或预计算的叶片方向先验获得
- 与你的条带卷积（Strip Context Block）配合使用，形成”结构感知”的完整方案

### 2.3 实例级空间注意力

| 方法 | 来源 | 核心思想 | 参数量 | 适用性 |
|---|---|---|---:|---|
| Instance Attention | arXiv 2024 | 针对每个实例生成独立的注意力图 | 中 | 适合 mask head |
| Prototype Attention | YOLO 内置 | 原型 mask 加权 | 小 | 已有 |
| Mask-Guided Attention | CVPR 2024 | 用 mask 引导注意力，形成闭环 | 小 | 适合你的 COB Loss |

**对你的适用性**：⭐⭐⭐⭐  
实例级空间注意力与你的 COB Loss 天然互补：COB Loss 从损失函数层面约束边界，实例级注意力从特征层面增强边界区域的表达。可以考虑在 mask head 前加入一个轻量 mask-guided attention。

### 2.4 轻量空间注意力的总结建议

| 模块 | 成本 | 推荐位置 | 预期收益 |
|---|---:|---|---|
| ASFF | 低 | neck 多尺度融合 | 多尺度自适应 |
| SA-Gate | 极低 | P2-Lite 路径 | 低光区域增强 |
| Strip Attention | 低 | P3/P4 特征图 | 条带遮挡建模 |
| Mask-Guided Attention | 低中 | mask head 前 | 边界精修 |

**核心建议**：不要堆叠多个空间注意力模块。对于第一篇论文，最推荐的是 **Strip Attention + COB Loss** 的组合——前者从结构层面建模条带遮挡，后者从损失层面约束凹陷边界，两者形成”结构+监督”的完整闭环。
