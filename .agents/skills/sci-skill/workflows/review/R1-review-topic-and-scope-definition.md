...
stage_id: R1
paper_family: review
stage_name_en: Review Topic and Scope Definition
stage_name_zh: 综述主题与范围确定
next_stage: R2
...
# R1 - Review Topic and Scope Definition

## Purpose

Compress a broad field into a bounded, searchable, and evaluable review topic.
...
```yaml
  - id: topic
    label_zh: 研究领域或综述主题
    ask_zh: 你准备综述哪个研究领域或主题？
    reply_format_zh: 研究领域或综述主题：
  - id: scope
    label_zh: 时间范围与学科边界
    ask_zh: 你目前设想的时间范围、研究对象、学科边界和文献类型是什么？
    reply_format_zh: 时间范围与学科边界：
  - id: databases
    label_zh: 拟检索数据库
    ask_zh: 你可以使用哪些数据库？不确定可写“暂无”。
    reply_format_zh: 拟检索数据库：
  - id: keywords_sources
    label_zh: 关键词或代表文献
    ask_zh: 你目前有哪些核心关键词或代表文献？
    reply_format_zh: 关键词或代表文献：
```
...
Generate multiple review topics and define scope, core questions, document types, classification logic, expected contribution, and whether a narrative review, systematic review, or meta-analysis is appropriate.
...
Narrow an existing topic by time, object, keywords, document type, database, inclusion and exclusion criteria. Test whether the review question can guide screening.
...
- If passed: update the state card and move to `R2`.
- If partially passed or failed: remain in `R1`, list at most three critical gaps, and provide executable repair tasks.
