...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    Extends detection loss with instance segmentation loss, semantic segmentation loss,
    optional BLF (Barrier Lyapunov Function) loss for amodal segmentation, and optional
    ellipse shape prior loss for improved mask quality.

    Attributes:
        overlap (bool): Whether to use overlapping masks.
        bcedice_loss (BCEDiceLoss): Combined BCE and Dice loss for semantic segmentation.
        blf_loss_fn (BLFLoss): Barrier Lyapunov Function loss for amodal constraints.
        ellipse_param_weight (float): Weight for ellipse parameter consistency loss.
        ellipse_dice_weight (float): Weight for soft ellipse Dice prior loss.
        ellipse_softness (float): Boundary sharpness for soft ellipse rasterization.
    """

    def __init__(
        self, model: torch.nn.Module, tal_topk: int = 10, tal_topk2: int | None = None
    ):  # model must be de-paralleled
        """Initialize v8SegmentationLoss with detection, segmentation, BLF, and ellipse prior losses.

        Args:
            model (torch.nn.Module): The YOLO model to compute losses for.
            tal_topk (int): Number of top-k anchors for task-aligned assignment.
            tal_topk2 (int | None): Secondary top-k for dual assignment strategy.
        """
        ...
        # Ellipse shape prior (disabled by default, set weight > 0 to enable)
    ...
    def single_mask_loss(
        ...
        """Compute instance segmentation loss for a single image, with optional amodal support.

        When gt_mask has 2 channels (visible + amodal), computes separate BCE losses for
        visible and amodal predictions, plus a BLF loss term to enforce the geometric
        constraint that amodal masks must fully contain visible masks.

        Args:
            gt_mask (torch.Tensor): Ground truth mask, shape (N, H, W) or (N, 2, H, W) for amodal.
            pred (torch.Tensor): Predicted mask coefficients of shape (N, 32).
            proto (torch.Tensor): Prototype masks of shape (32, H, W).
            xyxy (torch.Tensor): Ground truth bounding boxes in xyxy format, normalized to [0, 1].
            area (torch.Tensor): Area of each ground truth bounding box.
        ...
        # Support amodal segmentation (gt_mask with 2 channels: visible + amodal)
    ...
    def _fit_ellipse_from_soft_masks(masks: torch.Tensor, eps: float = 1e-6) -> tuple[torch.Tensor, ...]:
        """Extract centroid and 2x2 covariance matrix from soft masks using image moments.

        Avoids eigendecomposition for gradient stability. The covariance matrix captures
        ellipse shape information (axis lengths and orientation) without explicit parameterization.

        Args:
            masks (torch.Tensor): Soft masks of shape (N, H, W) with values in [0, 1].
            eps (float): Numerical stability term to avoid division by zero.

        Returns:
            tuple[torch.Tensor, ...]: (cx, cy, cov) where cx, cy are centroids of shape (N,)
                and cov is covariance matrix of shape (N, 2, 2).
        """
        n, h, w = masks.shape
    ...
    def _soft_ellipse_mask(
        ...
        """Render ellipse parameters into a differentiable soft ellipse mask.

        Uses the implicit ellipse equation d<=1 for interior points, with sigmoid((1-d)*softness)
        to convert hard boundaries into smooth boundaries for backpropagation.

        Args:
            cx (torch.Tensor): Ellipse center x-coordinate of shape (N,).
            cy (torch.Tensor): Ellipse center y-coordinate of shape (N,).
            a (torch.Tensor): Semi-major axis length of shape (N,).
            b (torch.Tensor): Semi-minor axis length of shape (N,).
            theta (torch.Tensor): Rotation angle in radians of shape (N,).
            h (int): Output mask height.
            w (int): Output mask width.
            softness (float): Boundary sharpness parameter.
            eps (float): Numerical stability term.

        Returns:
            (torch.Tensor): Soft ellipse mask of shape (N, H, W) with values in (0, 1).
        """
        n = cx.shape[0]
        yy, xx = torch.meshgrid(
    ...
    def _ellipse_shape_prior_loss(
        ...
        """Compute ellipse-based shape prior loss for improved mask quality.

        Consists of: (1) normalized L1 centroid loss between predicted and GT ellipse centers,
        (2) covariance matrix matching loss (MSE on 2x2 matrices, scale-normalized),
        (3) Dice loss on the cropped mask region.

        Args:
            gt_mask (torch.Tensor): Ground truth mask.
            pred (torch.Tensor): Predicted mask coefficients.
            proto (torch.Tensor): Prototype masks.
            xyxy (torch.Tensor): Bounding box in xyxy format.

        Returns:
            (torch.Tensor): Ellipse shape prior loss (scalar, 0 if weights are 0).
        """
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            pred_prob = pred_logits.sigmoid().clamp(1e-4, 1.0 - 1e-4)
            gt_prob = gt_mask_f.clamp(1e-4, 1.0)
            ...
            cx_p, cy_p, cov_p = self._fit_ellipse_from_soft_masks(pred_crop, self.ellipse_eps)

            with torch.no_grad():
                cx_g, cy_g, cov_g = self._fit_ellipse_from_soft_masks(gt_crop, self.ellipse_eps)

            # 1. Centroid loss (Normalized L1)
            center_loss = (cx_p - cx_g).abs() / max(w_c, 1) + (cy_p - cy_g).abs() / max(h_c, 1)

            # 2. Shape prior loss (Frobenius Norm on Covariance Matrices)
            cov_loss = F.mse_loss(cov_p, cov_g, reduction="none").sum(dim=(1, 2))
            scale_factor = (max(w_c, 1) * max(h_c, 1)) ** 2
            cov_loss = cov_loss / scale_factor

            param_loss = (center_loss + cov_loss).mean()

            # 3. Dice loss on cropped region
            inter = (pred_crop * gt_crop).sum(dim=(1, 2))
    ...
    def calculate_segmentation_loss(
        ...
        """Calculate instance segmentation loss with optional ellipse shape prior.

        Args:
            fg_mask (torch.Tensor): Positive anchor mask of shape (BS, N_anchors).
            masks (torch.Tensor): Ground truth masks.
            target_gt_idx (torch.Tensor): GT index for each anchor of shape (BS, N_anchors).
            target_bboxes (torch.Tensor): GT bounding boxes for each anchor of shape (BS, N_anchors, 4).
            batch_idx (torch.Tensor): Batch indices of shape (N_labels_in_batch, 1).
            proto (torch.Tensor): Prototype masks of shape (BS, 32, H, W).
            pred_masks (torch.Tensor): Predicted mask coefficients of shape (BS, N_anchors, 32).
            imgsz (torch.Tensor): Input image size tensor of shape (2).

        Returns:
            (torch.Tensor): Instance segmentation loss for the batch.
        """
        ...
        for i, single_i in enumerate(zip(fg_mask, target_gt_idx, pred_masks, proto, mxyxy, marea, masks)):
            ...
            if fg_mask_i.any():
                ...
                # Add ellipse shape prior (0 when weights are 0)
