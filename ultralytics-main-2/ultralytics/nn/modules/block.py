...
# =============================================================================
#  Custom modules: Depthwise Separable, Fusion, Attention, OverLoCK, Neck Gate
# =============================================================================
...
class DWSConv(nn.Module):
    ...
    Splits standard convolution into depthwise (per-channel spatial) and pointwise (1x1 channel mixing)
    stages. Falls back to standard Conv when kernel size is 1 since depthwise is unnecessary.

    Methods:
        forward: Apply depthwise separable convolution to input tensor.
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        """Initialize DWSConv with depthwise and pointwise branches.
        ...
        if k == 1:
            self.dw = None
            self.pw = Conv(c1, c2, 1, s, p, g, d, act)
        else:
...
class DWBottleneck(nn.Module):
    ...
    Replaces the internal 3x3 standard convolution with a depthwise separable convolution
    (DWConv) while keeping the 1x1 projection unchanged. Reduces parameters and FLOPs
    compared to standard Bottleneck.

    Methods:
        forward: Apply bottleneck with depthwise convolution to input tensor.
    ...
    def __init__(self, c1, c2, shortcut=True, k=3, e=0.5):
        """Initialize DWBottleneck with depthwise convolution.
        ...
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = DWConv(c_, c2, k, 1)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        """Apply bottleneck with depthwise convolution to input tensor.
...
class C3k2DW(nn.Module):
    """C3k2 variant using depthwise separable bottlenecks.

    Drop-in replacement for C3k2 that uses DWBottleneck instead of standard Bottleneck.
    Maintains the same YAML interface while reducing parameters and computation.

    Methods:
        forward: Apply C3k2DW pipeline with depthwise bottlenecks.

    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels.
        n (int): Number of DWBottleneck blocks.
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """Initialize C3k2DW with depthwise bottlenecks.
        ...
        self.m = nn.Sequential(*(DWBottleneck(c_, c_, shortcut=shortcut, e=1.0) for _ in range(n)))

    def forward(self, x):
        """Apply C3k2DW pipeline to input tensor.
...
class DSConv(nn.Module):
    """Depthwise Separable Convolution variant using Conv building blocks.

    Composes a depthwise convolution (groups=c1) with a pointwise convolution (groups=1)
    using the standard Conv class for consistent BN and activation handling.
    ...
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, d=1, act=True):
        """Initialize DSConv with depthwise and pointwise Conv branches.
...
class WeightedFusion(nn.Module):
    """Two-path weighted feature fusion module.

    Blends two feature maps using learned scalar weights instead of concatenation.
    Weights are initialized with shallow=2.0, deep=1.0 bias and normalized via ReLU
    to ensure non-negative contributions.

    Methods:
        forward: Fuse two feature maps with learned weights.

    Args:
        in_ch (int): Number of input channels (both paths must have same channels).

    Returns:
        (torch.Tensor): Weighted sum of two input feature maps.
    """

    def __init__(self, in_ch):
        """Initialize WeightedFusion with learnable weight parameters.
    ...
    def forward(self, x):
        """Fuse two feature maps with learned weights.

        Args:
            x (list[torch.Tensor]): List of two tensors [x_shallow, x_deep].

        Returns:
            (torch.Tensor): Weighted sum tensor of same shape as inputs.
        """
...
class MyAdaptivePool(nn.Module):
    """Custom adaptive pooling using avg_pool2d with scale factor.

    Performs average pooling with a fixed kernel size and stride determined by scale_factor,
    as an alternative to adaptive_avg_pool2d for spatial downsampling in skip connections.

    Methods:
        forward: Apply average pooling with scale factor.

    Args:
        in_ch (int): Number of input channels.
        scale_factor (int): Pooling kernel size and stride.

    Returns:
        (torch.Tensor): Downsampled tensor of shape (B, C, H/s, W/s).
    """

    def __init__(self, in_ch, scale_factor):
        """Initialize MyAdaptivePool with fixed scale factor.
    ...
    def forward(self, x):
        """Apply average pooling with scale factor.
...
class MyChannelAttention(nn.Module):
    """SE-style channel attention module.

    Applies squeeze-and-excitation attention: global average pooling followed by a
    two-layer MLP that produces per-channel attention weights via sigmoid.
    ...
    Returns:
        (torch.Tensor): Channel-attended tensor of same shape as input.
    """

    def __init__(self, in_ch, reduction=16):
        """Initialize MyChannelAttention with SE-style MLP.
    ...
    def forward(self, x):
        ...
        Returns:
            (torch.Tensor): Channel-attended tensor of shape (B, C, H, W).
        """
        b, c, _, _ = x.shape
...
class ScalarAttention(nn.Module):
    """Scalar gate attention module.

    Produces a single scalar gate value per sample using global average pooling followed
    by a linear layer and sigmoid. Multiplies the input by this scalar, effectively
    providing a learned skip-connection weight.

    Methods:
        forward: Apply scalar gate attention to input tensor.

    Args:
        in_ch (int): Number of input channels.

    Returns:
        (torch.Tensor): Scalar-attended tensor of same shape as input.
    """

    def __init__(self, in_ch):
        """Initialize ScalarAttention with scalar gate MLP.
        ...
        super().__init__()
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(nn.Linear(in_ch, 1, bias=False), nn.Sigmoid())

    def forward(self, x):
        """Apply scalar gate attention to input tensor.

        Args:
            x (torch.Tensor): Input tensor of shape (B, C, H, W).

        Returns:
            (torch.Tensor): Scalar-attended tensor of shape (B, C, H, W).
        """
        b, c, _, _ = x.shape
...
class ContMix(nn.Module):
    """Context-Mixing Dynamic Convolution from CVPR 2025 OverLoCK.

    Generates spatially-varying dynamic convolution kernels by mixing global context with
    local features. For each spatial token: (1) context is pooled to an SxS grid producing
    region descriptors, (2) descriptors are broadcast to input resolution, (3) local features
    are concatenated with context, (4) K-squared dynamic kernel weights are generated per
    position via MLP, (5) applied via unfold + weighted sum, plus a static depthwise conv
    residual for stability.

    Methods:
        forward: Apply context-mixing dynamic convolution to input tensor.

    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels.
        context_size (int): Context pooling grid size S.
        kernel_size (int): Dynamic kernel size K.

    Returns:
        (torch.Tensor): Output tensor of shape (B, c2, H, W).
    """

    def __init__(self, c1, c2, context_size=3, kernel_size=5):
        """Initialize ContMix with context-mixing dynamic convolution.
        ...
        # Context pooling and descriptor generation
        self.ctx_pool = nn.AdaptiveAvgPool2d(context_size)
        ...
        # Dynamic kernel generator: local + context -> K*K weights per position
        self.kernel_gen = nn.Sequential(
            nn.Conv2d(c1 * 2, c1, 1, bias=False),
            nn.BatchNorm2d(c1),
            nn.SiLU(),
            nn.Conv2d(c1, kernel_size * kernel_size, 1, bias=False),
        )

        # Static depthwise conv residual
        self.static_dw = nn.Sequential(
    ...
    def forward(self, x):
        """Apply context-mixing dynamic convolution to input tensor.
        ...
        # Broadcast context to input resolution
        ctx = F.interpolate(ctx, size=(H, W), mode="nearest")

        # Generate dynamic kernel weights
        local = F.adaptive_avg_pool2d(x, 1).expand_as(x)
        kernel_input = torch.cat([local, ctx], dim=1)
        kernels = self.kernel_gen(kernel_input)  # [B, K*K, H, W]
        kernels = kernels.softmax(dim=1)

        # Apply dynamic convolution via unfold
        x_unfold = F.unfold(x, kernel_size=self.k, padding=self.pad)  # [B, C*K*K, H*W]
        x_unfold = x_unfold.view(B, C, self.k * self.k, H, W)
        kernels = kernels.unsqueeze(1)  # [B, 1, K*K, H, W]
        out = (x_unfold * kernels).sum(dim=2)  # [B, C, H, W]

        # Add static depthwise residual
        out = out + self.static_dw(x)

        return self.proj(out)


class OverLoCKBlock(nn.Module):
    """OverLoCK-style residual block combining ContMix token mixing with FFN channel mixing.

    Drop-in replacement for Bottleneck in YOLO CSP structures. Uses context-mixing
    dynamic convolution for token mixing and a standard FFN for channel mixing.

    Methods:
        forward: Apply OverLoCK block with token and channel mixing.

    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels.
        context_size (int): Context pooling grid size for ContMix.
        kernel_size (int): Dynamic kernel size for ContMix.
    ...
    def __init__(self, c1, c2, context_size=3, kernel_size=5, mlp_ratio=2.0):
        """Initialize OverLoCKBlock with ContMix and FFN.
        ...
        """
        super().__init__()
        self.token_mixer = ContMix(c1, c2, context_size, kernel_size)
    ...
    def forward(self, x):
        """Apply OverLoCK block with token and channel mixing.
...
class C3k2_XXFormer(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5, num_heads=8, attn_ratio=0.5, ffn_expansion=2.0):
        ...
        self.m = nn.ModuleList(
            XXFormerBlock(self.c, num_heads, attn_ratio, ffn_expansion) for _ in range(n)
        )
