...
SAVE_DIR = os.path.join(RESULTS_DIR, "14_roi_heatmap_lite")
...
def parse_candidate_class_ids(value):
    ...
    class_ids = set()
    name_to_id = {name: idx for idx, name in enumerate(SEG_CLASS_NAMES)}
    for part in text.split(","):
        ...
        if token.isdigit():
            ...
            class_ids.add(name_to_id[token])
        else:
            raise ValueError(f"Unknown candidate class '{token}'. Use ids, names, or 'all'.")
    return class_ids


def format_candidate_class_ids(class_ids):
...
def extract_yolo_instances(seg_model, img_path, img_w, img_h, candidate_class_ids=None):
    ...
    for mask_idx, result_mask in enumerate(results[0].masks):
        class_id = -1
        confidence = 0.0
        if boxes is not None and len(boxes) > mask_idx:
            class_id = int(boxes.cls[mask_idx].detach().cpu().item())
            confidence = float(boxes.conf[mask_idx].detach().cpu().item())
        if candidate_class_ids is not None and class_id not in candidate_class_ids:
            continue
            ...
            instances.append(
                {
                    ...
                    "mask": candidate,
                    "class_id": class_id,
                    "class_name": SEG_CLASS_NAMES[class_id] if 0 <= class_id < len(SEG_CLASS_NAMES) else "unknown",
...
VISIBLE_GT_LABELS = {"fully_visible", "partially_visible"}
...
def load_gt_points(label_path, w, h):
    ...
    for shape in data.get("shapes", []):
        ...
        if label not in VISIBLE_GT_LABELS:
            continue
        shape_points = shape.get("points") or []
        if not shape_points:
            continue
...
def match_masks_to_gt(masks, gt_points, img_w, img_h):
    ...
    mask_centers = [compute_flower_center(mask) for mask in masks]
    candidates = []

    for mask_idx, flower_center in enumerate(mask_centers):
        for gt_idx, gt_point in enumerate(gt_points):
            distance_px = float(np.linalg.norm((gt_point["norm_pt"] - flower_center) * img_wh))
            candidates.append((distance_px, mask_idx, gt_idx))
    ...
    for distance_px, mask_idx, gt_idx in candidates:
        if distance_px > MAX_GT_MATCH_DISTANCE_PX:
            break
        if mask_idx in used_masks or gt_idx in used_gt:
            continue
        used_masks.add(mask_idx)
        used_gt.add(gt_idx)
        matches.append((mask_idx, gt_idx))
...
class YOLOROIHeatmapDataset(Dataset):
    def __init__(
        ...
        self.index_stats = {
            "source_images": len(self.img_files),
            "images_with_candidates": 0,
            "candidate_masks": 0,
            "visible_gt_points": 0,
            "matched_samples": 0,
            "unmatched_candidate_masks": 0,
            "unmatched_visible_gt_points": 0,
        }
    ...
    def _index_samples(self):
        ...
        for img_file in tqdm(self.img_files, desc=desc, ncols=90):
            ...
            label_path = str(Path(self.label_dir) / Path(img_file).with_suffix(".json"))
            ...
            self.index_stats["candidate_masks"] += len(instances)
            self.index_stats["visible_gt_points"] += len(gt_points)
            self.index_stats["matched_samples"] += len(matches)
            self.index_stats["unmatched_candidate_masks"] += max(len(instances) - len(matches), 0)
            self.index_stats["unmatched_visible_gt_points"] += max(len(gt_points) - len(matches), 0)
            if instances:
                self.index_stats["images_with_candidates"] += 1

            for instance_idx, gt_idx in matches:
    ...
    def _build_sample(self, idx):
        ...
        return {
            ...
            "class_id": torch.tensor(meta["class_id"], dtype=torch.long),
            "confidence": torch.tensor(meta["confidence"], dtype=torch.float32),
        }
