...
def _get_depth_path(image_path: str, depth_dir: Path) -> Path:
    """
    根据 RGB 图像路径和深度图目录推导深度图完整路径。
    深度图文件名与原图相同，后缀统一为 .png。
    例：
        image_path = datasets/yolo/TRAIN/img001.jpg
        depth_dir  = datasets/depth_pic
        →            datasets/depth_pic/img001.png
    """
    stem = Path(image_path).stem  # 仅取文件名，不含后缀
    return depth_dir / f"{stem}.png"
...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        # ===== 新增：深度图目录初始化 =====
        depth_dir = (data or {}).get("depth_dir", None)
        if depth_dir is not None:
            self.depth_dir = Path(depth_dir)
            if not self.depth_dir.exists():
                raise FileNotFoundError(
                    f"depth_dir specified in data.yaml not found: {self.depth_dir}"
                )
            LOGGER.info(f"Depth directory loaded: {self.depth_dir}")
        else:
            self.depth_dir = None
            LOGGER.warning(
                "No 'depth_dir' found in data.yaml. "
                "Depth channel will be filled with zeros."
            )
        # ==================================
