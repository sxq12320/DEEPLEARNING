"""CITRUS-E fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V3.py.

Example: python 20260909_citrus_e_v3_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v3_suite import GUIDED, NAMES, ROOT, SUITES, TILE_FRACTION, TILE_PROBABILITY, YAML_DIR, select_names
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V3.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V3" / f"CITRUS_EV3_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    prepared_data = None
