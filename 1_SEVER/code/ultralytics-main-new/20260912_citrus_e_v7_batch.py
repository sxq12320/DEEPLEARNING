"""CITRUS-E V7 fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V7.py.

Example: python 20260912_citrus_e_v7_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v7_suite import FACTORS, NAMES, ROOT, RUN_OVERRIDES, SUITES, select_names, YAML_DIR
...
def initialization_recorder(checkpoint):
    ...
    def record(trainer):
        ...
        report = {
            ...
            "note": (
                "Class-count changes and new/replaced layers need not match. "
                "Equality alone is not feature equivalence."
            ),
        }
...
def check_deform_backend(device):
    """Exercise the required compiled operator before costly cache preparation.

    This is an operator compatibility check, not a GPU occupancy/locking guard.
    Never silently substitute a different convolution or disable determinism.
    """
    from ultralytics.nn.modules.citrus_e_v7 import EV7DeformBlock
    from ultralytics.utils.torch_utils import select_device

    target = select_device(device)
    try:
        with torch.random.fork_rng(devices=[] if target.type == "cpu" else [target.index or 0]):
            block = EV7DeformBlock(16).to(target)
            x = torch.zeros(1, 16, 8, 8, device=target, requires_grad=True)
            block(x).square().mean().backward()
            if not all(torch.isfinite(p.grad).all() for p in block.parameters() if p.grad is not None):
                raise RuntimeError("Nonfinite deformable-op gradients")
    except (ImportError, RuntimeError, NotImplementedError) as error:
        raise RuntimeError(
            "V7 deform arms need a matching torch/torchvision deform_conv2d build on this device. "
            "No fallback was applied. Inspect the original error; non-deform arms remain available."
        ) from error
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V7.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V7" / f"CITRUS_EV7R2_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    if any(FACTORS[NAMES.index(name)][1] for name in names):
        check_deform_backend(args.device)
    fine_data = prepare_multiscale_views(data, project / "_prepared_multiscale", args.imgsz)
    _, digest = write_protocol_snapshot(
        project,
        {
            "series": "CITRUS-E-V7",
            "implementation_revision": "EV7R2_20260913_phase_base_scale_order_fix",
            ...
            "factors": "S=stride-4 P2 detection scale; D=DCNv2 deformable detail; C=PMCE channel enhancement; G=boundary/neighbor geometry supervision (gains .5/.25)",
