"""SAGE-v6 fixed-protocol sequential training. For VS Code use RUN_SAGE_V6.py.

Example: python 20260905_citrus_sage_v6_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
def main():
    ...
    if int(config.get("channels", 3)) != 3:
        raise ValueError("SAGE-v6 paper-1 experiments use RGB only")
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_SAGE_V6.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "SAGE_series" / f"CITRUS_SAGE_V6_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    ...
    source_files = [
        ...
        ROOT / "ultralytics/nn/modules/citrus_sage_v6.py",
