"""Build/profile eight E V7 arms, optionally exercise a tiny real sliced-data training loop."""
...
from citrus_e_v7_suite import NAMES, RUN_OVERRIDES, YAML_DIR
...
def main():
    ...
    for name in NAMES:
        ...
        row = dict(
            ...
            untraced_dcn_conv_gflops640=(
                ...
                if model.model[-1].deform_detail else 0.0
            ),
        )
        results.append(row)
    ...
    if args.smoke_data:
        ...
        for name in (NAMES[0], NAMES[-1]):
            ...
            loaded.val(validator=EV6Validator, data=str(fine), imgsz=128, batch=2, workers=0,
                       device="cpu", plots=False, project=str(args.output / "STANDALONE_NOT_FORMAL"), name=name)
