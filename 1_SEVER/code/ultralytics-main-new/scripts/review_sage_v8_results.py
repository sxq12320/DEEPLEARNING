"""Read all historical CSVs and audit V8 without changing uploaded artifacts."""
...
V8 = RESULTS / "SAGE/CITRUS_SAGE_V8_ALL_300EP"
OUT = ROOT / "reports/citrus_e_20260907"
...
def main():
    ...
    for path in (ROOT / "0_orange_yaml").rglob("*.yaml"):
        try:
            ...
            continue
    v8 = [r for r in records if Path(r["path"]).parent.parent == V8]
    reference = v8[0]["args"]
    ...
    recorded = json.loads((V8 / "_protocol/implementation_sha256.json").read_text())
    sources = {}
    ...
    payload = dict(history=records, errors=errors, yaml_index=configs, v8=v8, source_matches=sources)
    ...
    print(
        json.dumps(
            dict(
                ...
                v8=[{k: v for k, v in r.items() if k not in {"args", "initialization", "loaded"}} for r in v8],
            ),
            indent=2,
        )
    )
