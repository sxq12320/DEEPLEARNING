"""Read uploaded E V3 and all historical CSVs; preserve original results and report caveats."""
...
from citrus_e_v3_suite import FACTORS, NAMES

RESULTS = ROOT.parents[1] / "results"
SOURCE = RESULTS / "E/E_V3/CITRUS_EV3_ALL_300EP"
OUT = ROOT / "docs/E_V4_RECONSTRUCTION_EVIDENCE"
...
def main():
    ...
    for path in sorted(RESULTS.rglob("results.csv")):
        try:
            ...
            cfg = ROOT / f"0_orange_yaml/E_V3_series/{name}.yaml"
    ...
    assert len(runs) == 8, f"Missing E V3 results: {len(runs)}; errors={errors}"
    ...
    effects = {}
    for axis, name in enumerate(("deep", "neck", "quality")):
        pairs = []
        for a in runs:
            if a["factors"][axis]:
                continue
    ...
    lines = [
        "# 全部历史运行索引",
        "",
        "_2026-09-10；同一 epoch 的 Mask AP50 / AP50–95，非跨实验公平排行榜。_",
        "",
        "---",
        "",
        "同内容 CSV 共 190 份；展示副本仍列出，以路径区分。参数、数据划分、初始化和评估器未完全相同的运行不能据此判定架构胜负。",
        "",
