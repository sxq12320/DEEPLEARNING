"""Interleaved V8 compute benchmark. Synthetic timings are not dataset accuracy or deployment FPS."""
...
from citrus_sage_v8_suite import SUITES, YAML_DIR
...
def main():
    ...
    parser.add_argument("--only", default=",".join(SUITES["all"]))
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.output.exists():
        raise FileExistsError(args.output)
    names = args.only.split(",")
    if set(names) - set(SUITES["all"]) or len(names) != len(set(names)) or not names:
        raise ValueError("Choose distinct V8 models")
