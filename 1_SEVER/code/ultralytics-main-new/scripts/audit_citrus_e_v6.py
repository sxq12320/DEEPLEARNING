"""Build/profile ten E V6 arms, optionally exercise a tiny real sliced-data training loop."""
...
from citrus_e_v6_suite import NAMES, RUN_OVERRIDES, YAML_DIR
...
def main():
    ...
    if args.smoke_data:
        ...
        for name in (NAMES[0], NAMES[1], NAMES[7], NAMES[9]):
            model = YOLO(str(YAML_DIR / (name + ".yaml")), verbose=False).load(str(ROOT / "yolo11n-seg.pt"))
            import importlib

            runner = importlib.import_module("20260911_citrus_e_v6_batch")
            ...
            standalone = loaded.val(
                validator=EV6Validator, data=str(fine), imgsz=128, batch=2, workers=0,
                device="cpu", plots=False, save_json=False, save_txt=False,
                project=str(args.output / "SMOKE_STANDALONE_NOT_FORMAL"), name=name,
            )
            payload["smoke"].append(
                dict(
                    ...
                    standalone_metrics=standalone.results_dict,
