"""CITRUS-E V6 fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V6.py.

Example: python 20260911_citrus_e_v6_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v6_suite import NAMES, ROOT, RUN_OVERRIDES, SUITES, select_names, YAML_DIR
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V6.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V6" / f"CITRUS_EV6_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    ...
    _, digest = write_protocol_snapshot(
        project,
        {
            ...
            "factors": "F=stride-2 fine/phase decode(+mask_ratio2); N=nwd_ratio .5; P=copy_paste .3 flip",
            "run_overrides": RUN_OVERRIDES,
            "validation": "common input/2 fresh GT and logit raster; paired long-edge640 after training",
    ...
    source_files = [
        ...
        ROOT / "scripts/generate_citrus_e_v6_yaml.py",
        ROOT / "eval_citrus_e_v5.py",
        ROOT / "protocols/citrus_e_v6.yaml",
