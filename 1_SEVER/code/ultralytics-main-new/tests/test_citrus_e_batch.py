"""Queue safety, mask checkpoint selection and initialization provenance for v8."""
...
from citrus_e_suite import NAMES, ROOT, YAML_DIR
from ultralytics import YOLO


@pytest.fixture
def batch_module():
    spec = importlib.util.spec_from_file_location("citrus_e_test_runner", ROOT / "20260907_citrus_e_batch.py")
...
def test_initialization_is_measured_before_training(batch_module, tmp_path):
    ...
    try:
        ...
        assert 0.1 < report["equal_fraction"] < 1
        assert "model.0.conv.weight" in report["equal_keys"]
...
def test_queue_is_sequential_locked_and_interruptible(batch_module, tmp_path, monkeypatch, interrupt):
    ...
    monkeypatch.setattr(batch_module, "prepare_views", lambda data, *args: data)
    monkeypatch.setattr(batch_module, "finish_paired_evaluation", lambda *args: None)
    calls, active = [], []
...
def test_evaluation_retry_does_not_overwrite_or_repeat(batch_module, tmp_path, monkeypatch):
    import eval_citrus_sliced

    partial = tmp_path / "paired_sliced_eval"
    partial.mkdir()
    (partial / "interrupted.txt").write_text("keep")
    calls = []

    def fake_eval(weights, data, output, **kwargs):
        calls.append(output)
        output.mkdir()
        (output / "paired_metrics.json").write_text("{}")

    monkeypatch.setattr(eval_citrus_sliced, "evaluate", fake_eval)
    batch_module.finish_paired_evaluation(tmp_path, "unused.yaml", "cpu")
    batch_module.finish_paired_evaluation(tmp_path, "unused.yaml", "cpu")
    assert calls == [tmp_path / "paired_sliced_eval_retry1"]
    assert (partial / "interrupted.txt").read_text() == "keep"


@pytest.mark.parametrize("name", NAMES)
def test_real_trainer_calls_batch_callbacks_on_explicit_smoke_fixture(batch_module, tmp_path, monkeypatch, name):
    ...
    smoke.update(batch=2, imgsz=256, workers=0, mosaic=1.0, close_mosaic=0, plots=False)
    ...
    assert report["source_balanced_training"]["source_images"] == 4
    assert report["source_balanced_training"]["tile_probability"] == batch_module.TILE_PROBABILITY[name]
    assert report["val"]["images"] == 4
    inherited = json.loads((run / "initialization_transfer.json").read_text())
    assert 0.1 < inherited["equal_fraction"] < 1
