"""Queue safety, mask checkpoint selection and initialization provenance for v7."""
...
@pytest.fixture
def batch_module():
    spec = importlib.util.spec_from_file_location("sage_v7_test_runner", ROOT / "20260906_citrus_sage_v7_batch.py")
...
def test_real_trainer_calls_batch_callbacks_on_explicit_smoke_fixture(batch_module, tmp_path, monkeypatch):
    ...
    name = NAMES[-1]  # Exercise the local-context candidate arm and standard callbacks.
    monkeypatch.setattr(
        sys,
        "argv",
        [
