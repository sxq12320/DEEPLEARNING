"""CITRUS-E V10 fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V10.py.

Example: python 20260916_citrus_e_v10_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v10_suite import NAMES, ROOT, RUN_OVERRIDES, SUITES, select_names, YAML_DIR
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V10.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V10" / f"CITRUS_EV10_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    ...
    _, digest = write_protocol_snapshot(
        ...
        {
            "series": "CITRUS-E-V10",
            "implementation_revision": "EV10_20260916_structure_visibility",
            ...
            "factors": "S=contrast stem; D=early detail transport; B=64ch decoder; V=training-only visibility; O=cosine on09 only",
    ...
    source_files = [
        ...
        ROOT / "scripts/generate_citrus_e_v10_yaml.py",
        ROOT / "protocols/citrus_e_v10.yaml",
