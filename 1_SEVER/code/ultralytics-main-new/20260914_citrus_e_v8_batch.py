"""CITRUS-E V8 fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V8.py.

Example: python 20260914_citrus_e_v8_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v8_suite import NAMES, ROOT, RUN_OVERRIDES, SUITES, select_names, YAML_DIR
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V8.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V8" / f"CITRUS_EV8_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    ...
    _, digest = write_protocol_snapshot(
        ...
        {
            "series": "CITRUS-E-V8",
            "implementation_revision": "EV8_20260914_native_evidence",
            ...
            "factors": "B=context stage at C4; N=native P4 reconciliation; G=boundary/neighbor .5/.25; CP=copy-paste .3; D=dense fine decoder",
    ...
    source_files = [
        ROOT / "ultralytics/nn/modules/citrus_e_v8.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v3.py",
        ROOT / "scripts/generate_citrus_e_v8_yaml.py",
        ROOT / "protocols/citrus_e_v8.yaml",
