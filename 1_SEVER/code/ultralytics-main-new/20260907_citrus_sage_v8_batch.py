"""SAGE-v8 fixed-protocol sequential training. For VS Code use RUN_SAGE_V8.py.

Example: python 20260907_citrus_sage_v8_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_sage_v8_suite import NAMES, ROOT, SUITES, YAML_DIR, select_names
...
def main():
    ...
    if int(config.get("channels", 3)) != 3:
        raise ValueError("SAGE-v8 paper-1 experiments use RGB only")
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_SAGE_V8.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "SAGE_series" / f"CITRUS_SAGE_V8_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
