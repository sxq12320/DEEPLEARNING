"""Controlled batch runner for the task-specific CitrusC series.

Examples:
    python 20260828_citrus_c_batch.py --data /data/sxq/datasets/orange_yolo_2/data.yaml --suite smoke --epochs 3
    python 20260828_citrus_c_batch.py --data /data/sxq/datasets/orange_yolo_2/data.yaml --suite core --epochs 50
    python 20260828_citrus_c_batch.py --data /data/sxq/datasets/orange_yolo_2/data.yaml --suite architectures --epochs 50
    python 20260828_citrus_c_batch.py --data /data/sxq/datasets/orange_yolo_2/data.yaml --suite losses --epochs 50
    python 20260828_citrus_c_batch.py --data /data/sxq/datasets/orange_yolo_2/data.yaml --suite core --epochs 300 --only C03_dualproto_core
"""
...
DEFAULT_PROJECT = ROOT / "1_results" / "C_series" / "CITRUS_C_SCREEN_50EP"

TOPOLOGY = {"citrus_topology": 0.10}
B06_LOSS = {"citrus_boundary": 0.25, "citrus_query": 0.05}


@dataclass(frozen=True)
class Experiment:
    """One controlled architecture/loss hypothesis."""
...
ARCHITECTURES = (
    Experiment("C01_g10_clean", "01_c01_g10_clean_reproduction.yaml", {}, "exact G10 clean-data reproduction"),
    Experiment("C02_b06_anchor", "02_c02_b06_clean_anchor.yaml", B06_LOSS, "best B interaction anchor"),
    Experiment(
        "C03_dualproto_core",
        "03_c03_dualproto_core.yaml",
        TOPOLOGY,
        "semantic/detail prototypes plus one topology objective",
    ),
    Experiment(
        "C04_dualproto_hwd",
        "04_c04_dualproto_early_hwd.yaml",
        TOPOLOGY,
        "C03 plus early information-preserving downsampling",
    ),
    Experiment(
        "C05_dualproto_carafe",
        "05_c05_dualproto_carafe.yaml",
        TOPOLOGY,
        "C03 plus content-aware top-down reconstruction",
    ),
    Experiment(
        "C06_dualproto_hwd_carafe",
        "06_c06_dualproto_hwd_carafe.yaml",
        TOPOLOGY,
        "information-preserving encoder-neck-decoder path",
    ),
    Experiment(
        "C07_dualproto_context",
        "07_c07_dualproto_repcontext.yaml",
        TOPOLOGY,
        "C03 plus B-supported global context",
    ),
    Experiment(
        "C08_full_hypothesis",
        "08_c08_dualproto_full_hypothesis.yaml",
        TOPOLOGY,
        "full hypothesis with every factor independently controlled",
    ),
    Experiment("C09_g01_clean", "09_c09_g01_clean_reproduction.yaml", {}, "simpler G-family clean control"),
)

LOSSES = (
    Experiment("CL00_topology000", "03_c03_dualproto_core.yaml", {"citrus_topology": 0.0}, "no topology CE"),
    Experiment("CL01_topology005", "03_c03_dualproto_core.yaml", {"citrus_topology": 0.05}, "mild topology CE"),
    Experiment("CL02_topology010", "03_c03_dualproto_core.yaml", {"citrus_topology": 0.10}, "default topology CE"),
    Experiment("CL03_topology020", "03_c03_dualproto_core.yaml", {"citrus_topology": 0.20}, "strong topology CE"),
)
...
def experiments_for(suite: str) -> list[Experiment]:
    """Return a deterministic queue with appropriate controls."""
    if suite == "controls":
        return [ARCHITECTURES[index] for index in (0, 1, 8)]
    if suite == "core":
        return [ARCHITECTURES[index] for index in (2, 3, 4, 5, 6, 7)]
    if suite == "losses":
        return list(LOSSES)
    if suite == "smoke":
...
def prepare_data_yaml(source: Path, project: Path) -> Path:
    """Bind the runtime root without changing split membership."""
...
def completed(ledger: Path, run_name: str, run_dir: Path, epochs: int) -> bool:
    """Recognize both full-length and legitimate early-stopped completions."""
...
def main() -> None:
    """Run the selected C experiments sequentially on one GPU."""
    args = parse_args()
    ...
    if args.imgsz != 640:
        raise ValueError("Formal CitrusC comparisons lock imgsz=640.")
    ...
    for experiment, seed in queue:
        print(f"  {experiment.name:<28} seed={seed:<4} {experiment.yaml:<45} {experiment.losses}")
    if args.dry_run:
        return
    ...
    for index, (experiment, seed) in enumerate(queue, 1):
        ...
        if not yaml_path.is_file():
            raise FileNotFoundError(f"C model YAML not found: {yaml_path}")
