"""SAGE-v4r fixed-protocol sequential training. For VS Code use RUN_SAGE_V4.py.

Example: python 20260903_citrus_sage_v4r_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_sage_v4r_suite import NAMES, ROOT, SUITES, YAML_DIR, select_names
...
def main():
    ...
    if int(config.get("channels", 3)) != 3:
        raise ValueError("SAGE-v4r paper-1 experiments use RGB only")
    ...
    project = (
        (args.project or ROOT / "1_results" / "SAGE_series" / f"CITRUS_SAGE_V4R_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
