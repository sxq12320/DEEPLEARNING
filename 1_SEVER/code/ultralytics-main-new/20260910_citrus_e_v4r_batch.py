"""CITRUS-E fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V4.py.

Example: python 20260910_citrus_e_v4r_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v4r_suite import GUIDED, NAMES, ROOT, SUITES, TILE_FRACTION, TILE_PROBABILITY, YAML_DIR, select_names
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V4.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V4R" / f"CITRUS_EV4R_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    prepared_data = None
    ...
    _, digest = write_protocol_snapshot(
        project,
        {
            ...
            "input_recipe": "all eight arms: .5 local probability per source; fixed .6 windows; no guide",
...
def finish_paired_evaluation(directory, data, device):
    ...
    while True:
        ...
        if not output.exists():
            ...
            # V3 has no guide arm; this branch is retained only for the shared
            # evaluator's retry contract and remains inactive for all V3 names.
