...
class CitrusBoundaryQueryAux(nn.Module):
    """Training-only P2/P3 boundary and tiny-query supervision.

    This is the inference-free counterpart of :class:`CitrusBoundaryFusion`.
    It deliberately omits prototype refinement and the camouflage-contrast
    predictor: the completed S series showed that boundary plus query was the
    most stable topology recipe, while the full auxiliary-loss cocktail did not
    compound. Evaluation therefore follows the unchanged P3 prototype path.
    """
    ...
    def __init__(self, p2_channels: int, p3_channels: int):
        ...
        nn.init.constant_(self.query[-1].bias, -4.595)
    ...
    def forward(self, p2: torch.Tensor, p3: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Return P2-resolution visible-boundary and tiny-candidate logits."""
        ...
        boundary_logits = self.boundary_predictor(self.boundary_refine(boundary))
        return boundary_logits, query_logits
