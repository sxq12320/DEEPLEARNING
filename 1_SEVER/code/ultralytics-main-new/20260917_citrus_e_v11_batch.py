"""CITRUS-E V11 fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V11.py.

Example: python 20260917_citrus_e_v11_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v11_suite import NAMES, ROOT, RUN_OVERRIDES, SUITES, select_names, YAML_DIR
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V11.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V11" / f"CITRUS_EV11_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    ...
    _, digest = write_protocol_snapshot(
        ...
        {
            "series": "CITRUS-E-V11",
            "implementation_revision": "EV11_20260917_persistent_native_tiny_last_pair",
            "assignment": "00-04/07 standard TAL; 05/06/08/09/10/11 tiny NWD quality blend (.2); no extra anchors",
            "rgb_paired_cache_dedup": True,
            "input_recipe": "all arms: .5 global/.25 coarse/.25 fine; one draw per source",
            "factors": "H=persistent P2; R=Rep C3/C4/C5; N=native C4 route; Q=box factorization; A=tiny NWD-TAL; C=mask contrast; S=contrast stem00/09; 10/11=GAP/stable C5 attention replacement",
    ...
    source_files = [
        ROOT / "citrus_pr_diagnostics.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v9.py",
        ROOT / "ultralytics/utils/citrus_e_v9_loss.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v10.py",
        ROOT / "ultralytics/utils/citrus_e_v10_loss.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v11.py",
        ROOT / "ultralytics/utils/citrus_e_v11_loss.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v3.py",
        ROOT / "scripts/generate_citrus_e_v11_yaml.py",
        ROOT / "protocols/citrus_e_v11.yaml",
...
def finish_paired_evaluation(directory, data, device):
    ...
    for fraction, tag in ((0.6, "coarse"), (0.4, "fine")):
        ...
        if pointer.exists() and (directory / json.loads(pointer.read_text())["report"]).is_file():
            write_pr_diagnostics(directory / json.loads(pointer.read_text())["report"])
            continue
        attempt = 0
        while True:
