"""CITRUS-E fixed-protocol sequential training. For VS Code use RUN_CITRUS_E.py.

Example: python 20260907_citrus_e_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_suite import GUIDED, NAMES, ROOT, SUITES, YAML_DIR, TILE_FRACTION, TILE_PROBABILITY, select_names
...
def parse_args():
    ...
    parser.add_argument("--skip-sliced-eval", action="store_true", help="Explicitly skip post-training paired inference")
    ...
    for split, loader in (("train", trainer.train_loader), ("val", trainer.test_loader)):
        ...
        if hasattr(dataset, "source_groups"):
            summary["source_balanced_training"] = dict(
                source_images=len(dataset), source_instances=dataset.source_instances,
                cached_views=len(files), tile_probability=dataset.tile_probability,
            )
        summary[split] = {
            "images": len(files),
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_series" / f"CITRUS_E9_GUIDED_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    prepared_data = None
    ...
    if set(names) & GUIDED:
        ...
        guide_checkpoint = train_crop_guide(data, project / "_crop_guide/guide.pt", args.device,
                                           epochs=1 if args.epochs <= 3 else GUIDE_EPOCHS)
    ...
    _, digest = write_protocol_snapshot(
        project,
        {
            "series": "CITRUS-E9-GUIDED-20260908",
            "rgb_paired_cache_dedup": True,
            "input_recipe": "source-balanced; E06 context .75/global; other mixed arms .5/global",
    ...
    for name, seed in queue:
        ...
        try:
            ...
            input_trainer = {0.0: GlobalTrainingTrainer, 0.25: ContextTrainingTrainer,
                             0.5: SlicedTrainingTrainer}[TILE_PROBABILITY[name]]
...
def finish_paired_evaluation(directory, data, device):
    ...
    from eval_citrus_sliced import evaluate
    ...
    while True:
        ...
        if not output.exists():
            extra = {}
            if any(directory.name.startswith(name + "_seed") for name in GUIDED):
                extra["guide_checkpoint"] = directory.parent / "_crop_guide/guide.pt"
            evaluate(directory / "weights/best_mask.pt", data, output, device=device, fraction=TILE_FRACTION, **extra)
            break
        attempt += 1
