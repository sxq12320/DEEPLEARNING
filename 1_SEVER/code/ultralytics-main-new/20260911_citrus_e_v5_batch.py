"""CITRUS-E fixed-protocol sequential training. For VS Code use RUN_CITRUS_E_V5.py.

Example: python 20260911_citrus_e_v5_batch.py --data /data/orange/data.yaml --suite screen --epochs 300
...
from citrus_e_v5_suite import GUIDED, NAMES, ROOT, SUITES, TILE_FRACTION, TILE_PROBABILITY, YAML_DIR, select_names
from citrus_e_v5_suite import FINE_MODELS
...
def main():
    ...
    if args.dry_run:
        print("DRY RUN ONLY: no training. Set DRY_RUN=False in RUN_CITRUS_E_V5.py to train.", flush=True)
    ...
    project = (
        (args.project or ROOT / "1_results" / "E_V5" / f"CITRUS_EV5_{args.suite.upper()}_{args.epochs}EP")
        .expanduser()
        .resolve()
    )
    prepared_data = prepare_views(data, project / "_prepared_views", args.imgsz, TILE_FRACTION)
    fine_data = (
        prepare_multiscale_views(data, project / "_prepared_multiscale", args.imgsz)
        if set(names) & FINE_MODELS
        else None
    )
    _, digest = write_protocol_snapshot(
        project,
        {
            ...
            "input_recipe": "control .5 global/.5 coarse; fine .5 global/.25 coarse/.25 fine; one draw per source",
            "fine_models": sorted(FINE_MODELS),
    ...
    source_files = [
        ...
        ROOT / "scripts/generate_citrus_e_v5_yaml.py",
        ROOT / "eval_citrus_e_v5.py",
        ROOT / "protocols/citrus_e_v5.yaml",
        ...
        ROOT / "citrus_e_v5_suite.py",
        ROOT / "citrus_slicing.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v2.py",
        ROOT / "eval_citrus_e_v2.py",
        ROOT / "protocols/citrus_e_v4r.yaml",
        ROOT / "ultralytics/nn/modules/citrus_e_v3.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v4r.py",
        ROOT / "ultralytics/nn/modules/citrus_e_v4.py",
        ROOT / "scripts/generate_citrus_e_v4r_yaml.py",
        ROOT / "eval_citrus_e_v4r_calibration.py",
    ...
    for name, seed in queue:
        ...
        event = {
            ...
            "crop_policy": "uniform_multiscale" if name in FINE_MODELS else "fixed",
            "python": sys.version,
        ...
        try:
            ...
            training.update(
                data=str(fine_data if name in FINE_MODELS else prepared_data),
                project=str(project),
                name=run_name,
                epochs=args.epochs,
                device=args.device,
            ...
            if name in FINE_MODELS:
                input_trainer = MultiScaleTrainingTrainer
