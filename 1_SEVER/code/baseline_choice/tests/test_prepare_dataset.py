...
def create_tiny_source(root: Path) -> None:
    ...
    for split in ("train", "val", "test"):
        ...
        Image.new("RGB", (20, 10), (20, 80, 20)).save(
            image_dir / f"{split}_positive.jpg"
        )
        Image.new("RGB", (20, 10), (10, 10, 10)).save(
            image_dir / f"{split}_negative.jpg"
        )
...
def test_prepare_dataset_builds_all_layouts(tmp_path: Path) -> None:
    ...
    coco = json.loads(
        (output / "coco" / "annotations" / "instances_train.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_prepare_dataset_accepts_split_first_yolo_layout(tmp_path: Path) -> None:
    source = tmp_path / "source"
    output = tmp_path / "prepared"
    for split in ("train", "val", "test"):
        ...
        image_dir.mkdir(parents=True)
        label_dir.mkdir(parents=True)
        Image.new("RGB", (20, 10), (20, 80, 20)).save(image_dir / f"{split}.jpg")
    ...
    assert report["totals"] == {"images": 3, "instances": 3, "negative_images": 0}
    assert (output / "coco" / "images" / "train" / "train.jpg").is_file()
