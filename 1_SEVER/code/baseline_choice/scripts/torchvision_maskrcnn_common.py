...
class CocoMaskRCNNDataset:
    ...
    def __init__(
        self, annotation_path: Path, image_dir: Path, train: bool = False
    ) -> None:
        try:
    ...
    def __getitem__(self, index: int):
        ...
        if not image_path.is_file():
            raise FileNotFoundError(
                f"Image referenced by COCO JSON is missing: {image_path}"
            )
...
def validate_prepared_dataset(
    ...
    for split in splits:
        ...
        if category_ids != list(range(1, len(categories) + 1)):
            raise ValueError(
                f"{annotation_path}: category IDs must be consecutive and start from 1"
            )
        if category_names != list(class_names):
            raise ValueError(
                f"{annotation_path}: categories {category_names} != expected {list(class_names)}"
            )
        ...
        for annotation in annotations:
            ...
            if int(annotation["image_id"]) not in image_id_set:
                raise ValueError(
                    f"{annotation_path}: annotation {annotation_id} has an unknown image_id"
                )
            if int(annotation["category_id"]) not in category_ids:
                raise ValueError(
                    f"{annotation_path}: annotation {annotation_id} has an unknown category_id"
                )
            bbox = annotation.get("bbox", [])
            if len(bbox) != 4 or float(bbox[2]) <= 0 or float(bbox[3]) <= 0:
                raise ValueError(
                    f"{annotation_path}: annotation {annotation_id} has an invalid bbox"
                )
            if float(annotation.get("area", 0)) <= 0:
                raise ValueError(
                    f"{annotation_path}: annotation {annotation_id} has a non-positive area"
                )
            segmentation = annotation.get("segmentation")
            if not isinstance(segmentation, list) or not segmentation:
                raise ValueError(
                    f"{annotation_path}: annotation {annotation_id} has no polygon"
                )
            for polygon in segmentation:
                if len(polygon) < 6 or len(polygon) % 2:
                    raise ValueError(
                        f"{annotation_path}: annotation {annotation_id} has an invalid polygon"
                    )
...
def build_maskrcnn_model(
    ...
    try:
        ...
        raise RuntimeError(
            "Torchvision detection models are unavailable. Reinstall matching torch/torchvision builds."
        ) from exc
...
def move_targets_to_device(
    targets: Sequence[Mapping[str, Any]], device
) -> List[Dict[str, Any]]:
...
def train_one_epoch(
    ...
    progress.set_postfix(
        loss=f"{totals['loss'] / max(sample_count, 1):.4f}",
        time=f"{elapsed:.1f}s",
    )
...
def evaluate_model(
    ...
    with torch.inference_mode():
        ...
        for images, targets in progress:
            ...
            progress.set_postfix(predictions=len(predictions))
