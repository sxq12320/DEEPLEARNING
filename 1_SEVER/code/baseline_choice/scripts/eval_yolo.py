...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--retina-masks", action=argparse.BooleanOptionalAction, default=False)
    return parser
...
def main() -> None:
    ...
    image_count = 0
    for result in tqdm(results, total=len(image_index), desc=f"YOLO {args.split}", unit="image", dynamic_ncols=True):
        image_count += 1
